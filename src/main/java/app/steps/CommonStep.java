...
public class CommonStep {
    ...
    public static Car getCar(String number, int counter, int countOfTrying) {
        ...
        if (countOfTrying == 0) {
            throw new IllegalStateException("There was no compare trim link during several times");
        }
        ...
        if (carPage.isDisplayedCompareTrimsLink()) {
            carPage.clickOnCompareTrims();
        } else {
            Browser.enterUrl(PropertyManager.getConfigProperty("url"));
            return getCar(number, tempCounter, --countOfTrying);
        }
